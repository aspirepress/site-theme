...
add_action(
	...
	function () {
		...
		foreach ( $dependencies as $name => $path ) {
			wp_register_style( $name, get_template_directory_uri() . $path, array(), '3.37' );
		}
		wp_enqueue_style( 'akrabat3-style', get_stylesheet_uri(), array_keys( $dependencies ) );

		// Scripts.
		$dependencies = array( 'jquery' );
		wp_enqueue_script( 'bootstrap', get_template_directory_uri() . '/vendordir/js/bootstrap.min.js', array( 'jquery' ), '3.3.7', true );
		wp_enqueue_script( 'akrabat3-script', get_template_directory_uri() . '/script.js', array( 'bootstrap' ), '1.0.0', true );
	}
);
...
add_action(
	...
	function () {
		...
		add_theme_support( 'html5', array( 'search-form', 'comment-form', 'comment-list' ) ); // HTML 5 markup
	...
	function ( $query ) {
		if ( $query->is_search() ) {
			if ( isset( $_GET['post_type'] ) // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				&& ! empty( $_GET['post_type'] ) // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			) {
				$query->set( 'post_type', sanitize_key( wp_unslash( $_GET['post_type'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			}
		}
		return $query;
	},
	...
	<li <?php comment_class( 'row' ); ?> id="comment-<?php comment_ID(); ?>">
		...
		<?php endif; ?>
			...
			<div class="comment-meta meta">
				<a class="comment-link" href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
...
add_filter(
	...
	99,
	2
);  // Priority 99, Number of Arguments 2.
