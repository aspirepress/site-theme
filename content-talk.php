<article id="post-<?php the_ID(); ?>" <?php post_class( 'blog-post' ); ?>>
				...
				<?php
				$ap_post_id = get_the_ID();
				$event      = trim( get_post_meta( $ap_post_id, 'talk_event', true ) );
				$event_date = trim( get_post_meta( $ap_post_id, 'talk_date', true ) );
				$event_text = '';
				if ( $event ) {
					if ( $event_date ) {
						$event .= ", $event_date";
				...
				$notist_code = trim( get_post_meta( $ap_post_id, 'talk_code_notist', true ) );
				...
				foreach ( $akrabat_talk_type_fields as $key => $label ) {
					if ( false === stripos( $label, 'URL' ) ) {
						continue;
					}
					$label = str_replace( ' URL', '', $label );
					$value = trim( get_post_meta( $ap_post_id, $key, true ) );
					if ( empty( $value ) ) {
						continue;
					}
				...
				if ( ! is_singular() ) {
					...
					$speakerdeck_url = trim( get_post_meta( $ap_post_id, 'talk_url_speakerdeck', true ) );
					if ( $speakerdeck_url ) {
						$site = 'Slides';
					...
					$youtube_id = trim( get_post_meta( $ap_post_id, 'talk_youtube_id', true ) );
					if ( $youtube_id ) {
...
HTML;
					...
					$video_embed = trim( get_post_meta( $ap_post_id, 'talk_embed_video', true ) );
					if ( $video_embed ) {
						echo <<<HTML
                        <p>
                        <h4>Video</h4>
...
HTML;
					...
					if ( $notist_code ) {
						// $notist_code = 'JwEI4m';
						$title           = get_the_title( $ap_post_id );
						$notist_username = 'akrabat';

						$notist_embed_html    = get_post_meta( $ap_post_id, 'talk_notist_embed_html', true );
						$notist_embed_expires = get_post_meta( $ap_post_id, 'talk_notist_embed_expires', true );
						$notist_embed_aspect  = get_post_meta( $ap_post_id, 'talk_notist_embed_aspect', true );
						...
						if ( empty( $notist_embed_html ) || time() > $notist_embed_expires ) {
							...
							if ( $notist_embed_html ) {
								update_post_meta( $ap_post_id, 'talk_notist_embed_html', $notist_embed_html );
								update_post_meta( $ap_post_id, 'talk_notist_embed_expires', $notist_embed_expires );
								update_post_meta( $ap_post_id, 'talk_notist_embed_aspect', $notist_embed_aspect );
							}
						}
...
HTML;
						...
						$slides_embed = trim( get_post_meta( $ap_post_id, 'talk_embed_slides', true ) );
						if ( $slides_embed ) {
							echo <<<HTML
